    ...
    public partial class UCStudDetailed : UserControl
    {
        ...
        public void LoadStudentData(int i)
        {
            try
            {
                Db.OpenConnection();
                aisid = i;
                cmd = new MySqlCommand("SELECT * FROM student_data where aisid = @aisid", Db.GetConnection());
                ...
                cmd = new MySqlCommand("select id, relationship, name, photo from stud_relationship where relationshiptostud = @aisid", Db.GetConnection());
                cmd.Parameters.Add("@aisid", MySqlDbType.Int32).Value = aisid;
                ...
                ReadClassHistory();
                //medical
                cmd = new MySqlCommand("SELECT * FROM aisDb.stud_medical_info where medicalofstud = @aisid", Db.GetConnection());
                cmd.Parameters.Add("@aisid", MySqlDbType.Int32).Value = aisid;
        ...
        {
            //class history
            cmd = new MySqlCommand("select class_history.class_id, class_history.class_status, class.classname, class.careteacher, class.assistant, class.class_start, class.class_stat, employee_data.emp_fullname from aisDb.class_history inner join class on class.class_id = class_history.class_id inner join employee_data on employee_data.employeeid = class.careteacher where class_history.stud_id = @aisid;", Db.GetConnection());
            cmd.Parameters.Add("@aisid", MySqlDbType.Int32).Value = aisid;
        ...
        {
            //prevschoolinfo
            cmd = new MySqlCommand("SELECT name_of_school, country, grade, dateattended, language_of_instruction, extra_support, curriculum FROM student_previous_school_info where of_student = @aisid", Db.GetConnection());
            cmd.Parameters.Add("@aisid", MySqlDbType.Int32).Value = aisid;
        ...
        {
            //read documents
            cmd = new MySqlCommand("select docsname, docspath, docstype, docsdesc from document_students where owner_id = @owner_id", Db.GetConnection());
