    ...
    public partial class UCMaster : UserControl
    {
        ...
        private void LoadSchoolYear(string str)
        {
            if(str == "Ongoing")
            {
                ...
                DataTable dt = Query.Load(Query.Process.Master, new string[1] { str });
                if (dt.Rows.Count >= 1)
                {
                ...
                {
                    PopUp.Alert("Currently, there are no ongoing schoolyear", frmAlert.AlertType.Error);
                }
            }
        ...
        private void MasterDataValidation()
        {
            switch (_menu)
            {
                case NavSwitcher.Home:
                    ...
                    {
                        PopUp.Alert("Not valid academic year code!", frmAlert.AlertType.Error);
                    }
                    else if (Convert.ToByte(dropTerms.SelectedItem.ToString()) <= terms)
                    {
                        PopUp.Alert("You cannot start this academic year\non the same/previous terms!", frmAlert.AlertType.Error);
                    }
                    else
                    {
                ...
                case NavSwitcher.Proceed:
                    ...
                    {
                        NewUniqueKey = Query.GetRandomNumber(Query.Process.Master);
                        MenuSwitcher(NavSwitcher.Confirm);
                    }
                    else
                    {
                        PopUp.Alert("You entered the wrong key", frmAlert.AlertType.Error);
                    }
                    break;
                case NavSwitcher.Confirm:
