...
namespace AisInternalSystem
{
    public partial class UCEmployeeDetailed : UserControl
    {
        ...
        public void LoadEmployeeData()
        {
            ...
            {
                Db.OpenConnection();
                cmd = new MySqlCommand("select * from employee_data where emp_id = @emp_id", Db.GetConnection());
                    ...
                    while (reader.Read())
                    {
                        try
                        {
                            ...
                            if(ispursuingDegree == "YES")
                            {
                                ...
                                {
                                    PopUp.Alert("Error Occured", frmAlert.AlertType.Warning);
                                }
                            }
                            else
                            {
        ...
        {
            //read documents
            cmd = new MySqlCommand("select docsname, docspath, docstype, docsdesc from document_employee where owner_id = @owner_id", Db.GetConnection());
